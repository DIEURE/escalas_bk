...
public interface UsuarioRepository extends JpaRepository<Usuario, Long> {
	...
	// 🟢 Consulta paginada para a Data Table com filtros (Nome/Email, Perfil e
	// Ativo)
	@Query("""
	...
	// Para o Super Admin listar todos os pendentes de todas as congregações
	List<Usuario> findByAtivoFalseOrderByNomeAsc();

	// Para filtrar pendentes de uma congregação específica
	List<Usuario> findByAtivoFalseAndEmpresaIdOrderByNomeAsc(Long empresaId);

	// Para o SUPER_ADMIN listar todos os usuários do sistema
	List<Usuario> findAllByOrderByNomeAsc();

	// Para filtrar por uma empresa específica (Admin local ou Super Admin
	// filtrando)
